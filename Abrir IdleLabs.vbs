' Abre o IdleLabs sem a janela preta do cmd.
...
If sh.Run("cmd /c where npm >nul 2>nul", 0, True) <> 0 Then
  MsgBox "O Node.js nao esta instalado." & vbCrLf & _
         ...
         "Depois abra este arquivo de novo.", 48, "IdleLabs"
  WScript.Quit
End If

' primeira vez: instala com a janela visivel (ela fecha sozinha ao terminar)
If Not fso.FolderExists(pasta & "\node_modules") Then
  sh.Run "cmd /c title IdleLabs - primeira instalacao && echo Instalando o necessario, aguarde... && npm install", 1, True
End If
